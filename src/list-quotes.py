# Run this code on your paper or chapter folder to store on a csv file all the quotes, signaled with ";;". Remember to run `post-list-quotes.py` afterwards.
...
for subdir, dirs, files in os.walk(directory):
    for file in files:
        if file.endswith(".md"):
            ...
            # Convert the markdown to HTML to make it easier to extract words
            html = markdown.markdown(contents)

            # Extract all the words that start with ";;" from the HTML
            words = [word.translate(str.maketrans("", "", punctuation)) for word in html.split() if word.startswith(";;")]
